...
def softmax(attention_weights):
    appliedExponentials = torch.exp(attention_weights)
    sumForNormalization = torch.sum(appliedExponentials)
    return appliedExponentials / sumForNormalization

# Taylor softmax implementation with 4 expansion terms
def taylorSoftmax(attention_weights, num_iters, dimention=1):
    for (idx, val) in enumerate(attention_weights[0]):
        factorial = 1
        numerator = 1
        for j in range(1, num_iters):
            factorial *= j
            numerator = numerator + pow(val.item(), j)/factorial
        attention_weights[0][idx] = numerator
    sum = torch.sum(attention_weights)
    return attention_weights / sum

# SoftMargin Softmax implementation with 0.6 margin (supposedly optimized)
def softMarginSoftmax(attention_weights):
    appliedExponentials = torch.exp(attention_weights - 0.6)
    sumForNormalization = torch.sum(appliedExponentials)
    return appliedExponentials / sumForNormalization

# SoftMargin Taylor softmax implementation with a 0.6 implementation
def SM_Taylor_Softmax(attention_weights, num_iters):
    for (idx, val) in enumerate(attention_weights[0]):
        factorial = 1
        numerator = 1
        for j in range(1, num_iters):
            factorial *= j
            numerator = numerator + pow(val.item(), j)/factorial
        attention_weights[0][idx] = numerator-0.6
    sum = torch.sum(attention_weights)
    return attention_weights / sum
...
class AttnDecoderRNN(nn.Module):
    ...
    def forward(self, input, hidden, encoder_outputs):
        ...
        # REPLACE F.softmax() with softmax functions above to try different softmax approaches
...
def train(input_tensor, target_tensor, encoder, decoder, optimizer, criterion, max_length=MAX_LENGTH):
    ...
    sos_indexes = []
    for i in range(BATCH_SIZE): # Append batch size number of start of sentence tokens for sos indexes list
...
def main():
    ...
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, patience=5000, min_lr=0.00005, factor=0.5,
                                                           cooldown=20000)
    ...
    while iter_num < n_iters:
        ...
        # Get Batch_size number of translation pairs
        for i in range(BATCH_SIZE):
            tensors.append(tensors_from_pair(src_vocab, tgt_vocab, random.choice(train_pairs)))

        # Split translation pairs into input and target (output)
        for i in range(BATCH_SIZE):
        ...
        if iter_num % print_every == 0:
            ...
            print("LR = ", get_lr(optimizer), "Loss = ", loss)

...
class EncoderRNN(nn.Module):
    ...
    def forward(self, input, hidden):
        ...
        if self.firstRun==True:
            c_t=torch.zeros(bs, self.hidden_size).to(input.device)
            self.firstRun=False



        h_t = torch.squeeze(hidden)
        ...
        output, hidden = hidden_seq, (h_t, c_t)
...
class AttnDecoderRNN(nn.Module):
    ...
    def __init__(self, hidden_size, output_size, dropout_p=0.1, max_length=MAX_LENGTH):
        ...
        self.U_i = nn.Parameter(torch.Tensor(self.output_size, self.hidden_size))
        self.V_i = nn.Parameter(torch.Tensor(self.hidden_size, self.hidden_size))
        self.b_i = nn.Parameter(torch.Tensor(self.hidden_size))

        # f_t
        self.U_f = nn.Parameter(torch.Tensor(self.output_size, self.hidden_size))
        self.V_f = nn.Parameter(torch.Tensor(self.hidden_size, self.hidden_size))
        self.b_f = nn.Parameter(torch.Tensor(self.hidden_size))

        # c_t
        self.U_c = nn.Parameter(torch.Tensor(self.output_size, self.hidden_size))
        self.V_c = nn.Parameter(torch.Tensor(self.hidden_size, self.hidden_size))
        self.b_c = nn.Parameter(torch.Tensor(self.hidden_size))

        # o_t
        self.U_o = nn.Parameter(torch.Tensor(self.output_size, self.hidden_size))
        self.V_o = nn.Parameter(torch.Tensor(self.hidden_size, self.hidden_size))
        self.b_o = nn.Parameter(torch.Tensor(self.hidden_size))
    ...
    def forward(self, input, hidden, encoder_outputs):
        ...
        #LSTM
        bs, seq_sz, _ = output.size()
        hidden_seq = []

        if hidden is None:
            h_t, c_t = (
                torch.zeros(bs, self.hidden_size).to(output.device),
                torch.zeros(bs, self.hidden_size).to(output.device),
            )
        else:
...
def train(input_tensor, target_tensor, encoder, decoder, optimizer, criterion, max_length=MAX_LENGTH):
    ...
    encoder_optimizer=encoder.train()
    decoder_optimizer=decoder.train()

...
class AttnDecoderRNN(nn.Module):
    ...
    def forward(self, input, hidden, encoder_outputs):
        ...
        if hidden[0].dim() == 2:
            ...
            output=F.relu(self.attention_layers(torch.cat((embedding[0], torch.matmul(attention_weights,    encoder_outputs).unsqueeze(0)[0]), 1)).unsqueeze(0))
            ...
            output = F.relu(self.attention_layers(
